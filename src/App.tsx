            ...
            <div className="bg-[#0A0A0A] p-8 rounded-3xl border border-slate-800/50 space-y-8 shadow-2xl shadow-black/50 backdrop-blur-sm">
              ...
              <div className="w-full relative">
                ...
                >
                  ...
                  <ChevronDown className={`w-6 h-6 text-[#8E9299] transition-transform duration-300 ${isDropdownOpen ? 'rotate-180' : ''}`} />
                </button>
                {isDropdownOpen && (
                  <div className="absolute top-full left-0 right-0 mt-3 bg-[#1C1D21] border border-slate-800 rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 max-h-64 overflow-y-auto custom-scrollbar">
                    ...
                    >
                      <span className="font-medium group-hover:text-blue-400 transition-colors">Endless Flight</span>
                      <span className="text-[10px] bg-slate-800 px-2 py-1 rounded text-slate-400 uppercase">Zen</span>
                    </button>
                    {SONGS.map(song => (
                      <button 
                        key={song.id} 
                      ...
                      >
                        <span className="font-medium group-hover:text-blue-400 transition-colors truncate mr-2">{song.title}</span>
                        <span className={`text-[10px] px-2 py-1 rounded uppercase ${
                          song.type === 'easy' ? 'bg-green-900/30 text-green-400' : 
                          song.type === 'medium' ? 'bg-amber-900/30 text-amber-400' : 
