...
export const loadProfile = (): PlayerProfile => {
  ...
  if (stored) {
    try {
      ...
      if (profile.currency === undefined) profile.currency = 0;
      if (!profile.ownedPlanes) profile.ownedPlanes = ['default'];
      if (!profile.selectedPlane) profile.selectedPlane = 'default';
